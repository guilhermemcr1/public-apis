...
Mantenha `PUBLIC_APIS_BIND=127.0.0.1` e `PUBLIC_APIS_PORT=18080` enquanto Laravel e Go estiverem rodando juntos. Se houver reverse proxy, configure em `.env` somente o IP ou CIDR dele em `TRUSTED_PROXY_CIDRS`; não confie em redes maiores que o necessário.
...
curl -fsS 'http://127.0.0.1:18080/getip?format=json'
curl -fsS 'http://127.0.0.1:18080/getuuid?version=7'
curl -fsS http://127.0.0.1:18080/docs/getip
```
...
1. Troque o upstream do reverse proxy da porta Laravel para `127.0.0.1:18080`.
...
4. Opcionalmente altere `PUBLIC_APIS_PORT` para `8080`; mantenha `18080` se o reverse proxy já estiver configurado e protegido.
